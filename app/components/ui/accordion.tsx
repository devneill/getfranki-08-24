...
>(({ className, children, ...props }, ref) => (
	<AccordionPrimitive.Header className="flex">
		<AccordionPrimitive.Trigger
			ref={ref}
			className={cn(
				'group flex flex-1 items-center justify-between py-5 font-semibold transition-all xl:text-lg [&[data-state=open]>svg]:rotate-180',
				className,
			)}
			{...props}
		>
			<div className="transition-all group-data-[state=open]:text-primary">
				{children}
			</div>
			<Icon
				name="chevron-down"
